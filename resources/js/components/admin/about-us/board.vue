<template>
    <div>
        ...
        <div>
            <Dialog header="Add Board Member" v-model:visible="show_add_modal"
            ...
            >
                ...
                <template #footer>
                    ...
                    <Button :loading="loadingAdd" label="Save Board Member" @click="createBoard"/>
                </template>
            </Dialog>
        </div>
        ...
        <!--        edit-->
        <div>
            <Dialog header="Edit Board Member" v-model:visible="show_edit_modal"
                    :breakpoints="{'960px': '75vw', '640px': '100vw'}" :style="{width: '50vw'}"
            >
                <div style="padding-top: 10px">
                    <div>
                ...
                <template #footer>
                    ...
                    <Button :loading="loadingEdit" label="Edit Board Member" @click="editBoardMember"/>
                </template>
            </Dialog>
        </div>
        <!--        edit-->
        <div>
            <DataTable :value="board" responsiveLayout="scroll">
...
export default {
    ...
    methods: {
        ...
        editBoardMember() {
            ...
            axios.post("/api/edit-board", theData)
                .then(res => {
                    this.successToast("Board member details changed")
                    this.loadingEdit = false
                    this.show_edit_modal = false
                    for (let i in this.board) {
                        if (this.board[i].id === this.formEdit.id) {
                            this.board[i].name = res.data.name;
                            this.board[i].title = res.data.title;
                            this.board[i].abt = res.data.abt;
                            this.board[i].img = res.data.img;
                            break;
                        }
                    }
                }).catch(err => {
        ...
        createBoard() {
            ...
            if (this.imagesArray !== null) {
                ...
                axios.post("/api/create-board", formData)
                    .then(res => {
                        this.successToast("Board member saved")
        ...
        deleteBoardMember(id) {
            axios.delete(`/api/delete-board/${id}`).then(res => {
                this.successToast(res.data)
                this.board.splice(this.board.findIndex(item => item.id === id), 1)
            }).catch(err => {
                this.errorToast("An error occurred!")
            });
        }
